...
from .data_input_agent import data_agent
...
2. If the user wants to provide a picture of a document call 'data_input_agent'
3. If the user wants to ask a question and get the response from the database call the 'query_agent' """,
    # We wrap the sub-agents in `AgentTool` to make them callable tools for the root agent.
    tools=[AgentTool(data_agent), AgentTool(query_agent)]
)
